...
if check_password():
    # --- LOGO NA SIDEBAR ---
    # Tenta carregar o arquivo Logo.jpg que você enviou
        ...
        try:
            ...
            st.sidebar.markdown(
                ...
                <div style="text-align: center;">
                    <img src="data:image/jpeg;base64,{bin_str}" style="width: 100%; border-radius: 10px; margin-bottom: 20px;">
                </div>
                """,
                unsafe_allow_html=True
            )
        except:
            st.sidebar.title("Inove")
    else:
        st.sidebar.title("Inove")
    ...
    if not df.empty:
        ...
        mes_list = sorted(df['Mes_Ano'].unique(), reverse=True)
        mes_sel = st.sidebar.multiselect("Filtrar Mês:", mes_list, default=mes_list)
        ...
        with ce1:
            ...
            st.download_button("📥 Baixar Excel", data=buffer.getvalue(), file_name="Inove_Analitico.xlsx", use_container_width=True)
        with ce2:
